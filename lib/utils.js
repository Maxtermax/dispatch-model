...
module.exports = {
  extractDeep: function(data, deep) {
    return _.map(deep, (de) => {
      if(_.isArray(de)) return this.extractDeep(data, de);
      return _.pick(data, de);
    })
  },
  formarData: function(format = {}, data = {}) {
    if(_.isArray(data)) {
      return _.pick(data, _.keys(format));
    } else {
      _.map(_.keys(format), (key) => {
        if(_.isArray(format[key])) return this.extractDeep(data, format[key]);
  ...
  model: {
    ...
    createSure: function(data = {}, Model = {}) {
      ...
      return (new Promise((resolve, reject)=> {
          let requiresAttr = self.model.attributesRequired(Model);
          let hasRequires = _.every(requiresAttr, (required)=> data.hasOwnProperty(required));
          if(hasRequires) {
            let isValid = self.model.validateData(Model, data);
            if(isValid) {
