const notAllowResponse = require('./notAllow.js');
const forbiddenResponse = require('./forbidden.js');
const serverErrorResponse = require('./serverError.js');
const badRequestResponse = require('./badRequest.js');
const conflictResponse = require('./conflict.js');
const notFoundResponse = require('./notFound.js');
const utils = require('./utils.js');
const _ = require('lodash');


module.exports = function dispatchModel(Query, options = {}, format) {
  let req = this.req;
  let res = this.res || this;
  const endpoint = req.originalUrl;
  
  res.notAllow = res.notAllow || notAllowResponse;
  res.serverError = res.serverError || serverErrorResponse;
  res.forbidden = res.forbidden || forbiddenResponse;
  res.badRequest = res.badRequest || badRequestResponse;
  res.conflict = res.conflict || conflictResponse;
  res.notFound = res.notFound || notFoundResponse;
  ...
  return (
      ...
      let response = {
        data: utils.formarData(format, docs),
      ...
      function dispatch(error) {
        ...
        if(docs) {
          ...
          res.notFound({data: null, error: notFound, endpoint});
        }
      }//end dispatch
  	...
  	.catch(function(err) {
       function dispatchError() {
        ...
        const response = {data: null, endpoint};
        if(err.code === 11000 || (err.hasOwnProperty('originalError') && err.originalError.code === 11000)) {
          _.extend(response, {error: conflict});
          res.status(409);
          if(options.errors.conflict.hasOwnProperty('view')) return res.conflict(options.errors.conflict.view, response.error);//Render view in case of error
          return res.conflict(response);
          /*
